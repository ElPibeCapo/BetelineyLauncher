...
QJsonDocument requireDocument(const QByteArray& data, const QString& what)
{
    if (isBinaryJson(data)) {
        // Qt no soporta BSON ni otros formatos JSON binarios — QJsonDocument::fromJson()
        // los rechazaría de todas formas con un error críptico. Lanzar aquí da un mensaje
        // más claro al diagnóstico.
        throw JsonException(what + ": Invalid JSON. Binary JSON (BSON) is not supported");
    } else {
...
{
    const QString string = requireIsType<QString>(value, what);
    // Reject strings with null bytes or characters invalid in file paths
    // BUG 6 fix: también rechazar secuencias de path traversal (..)
    if (string.contains(QChar('\0')) || string.contains(QLatin1String(".."))) {
        throw JsonException(what + " contains invalid path characters");
    }
