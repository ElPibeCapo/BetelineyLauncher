...
void PackInstallTask::extractConfigs()
{
    ...
    m_extractFuture = QtConcurrent::run(QThreadPool::globalInstance(), QOverload<QString, QString>::of(BetelineyZip::extractDir), archivePath,
                                  ...
                                  const QMap<QString, QString>& toCopy)
{
    ...
    for (auto iter = toExtract.begin(); iter != toExtract.end(); iter++) {
        ...
        if (!BetelineyZip::extractDir(modPath, folderToExtract, extractToPath)) {
            // assume error
            return false;
        }
    }
    ...
    for (auto iter = toDecomp.begin(); iter != toDecomp.end(); iter++) {
        ...
        if (!BetelineyZip::extractFile(modPath, mod.decompFile, extractToPath)) {
