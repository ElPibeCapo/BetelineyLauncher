...
void Technic::SingleZipPackInstallTask::downloadSucceeded()
{
    ...
    m_packZip.reset(new BetelineyZip::ArchiveReader(m_archivePath));
    m_extractFuture =
        QtConcurrent::run(QThreadPool::globalInstance(), BetelineyZip::extractSubDir, m_packZip.get(), QString(""), extractDir.absolutePath());
