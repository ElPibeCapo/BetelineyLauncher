...
ModrinthPackExportTask::ModrinthPackExportTask(const QString& name,
                                               ...
                                               BetelineyZip::FilterFileFunction filter)
    : name(name)
    , version(version)
    , summary(summary)
...
void ModrinthPackExportTask::collectFiles()
{
    ...
    files.clear();
    if (!BetelineyZip::collectFileListRecursively(instance->gameRoot(), nullptr, &files, filter)) {
...
void ModrinthPackExportTask::buildZip()
{
    ...
    auto zipTask = makeShared<BetelineyZip::ExportToZipTask>(output, gameRoot, files, "overrides/", true);
