...
void LaunchController::decideAccount()
{
    ...
    if (!accounts->anyAccountIsValid()) {
        ...
        auto reply = CustomMessageBox::selectable(m_parentWidget, tr("No Accounts"),
                                                  tr("Para jugar Minecraft necesitás tener al menos una cuenta agregada (Premium o No-Premium). "
                                                     "¿Querés abrir el gestor de cuentas para agregar una ahora?"),
...
LaunchDecision LaunchController::decideLaunchMode()
{
    ...
    if (m_wantedLaunchMode == LaunchMode::Normal && m_accountToUse->accountType() != AccountType::Offline) {
    ...
    // ── BETELINEY: Cuentas No-Premium no necesitan verificar una cuenta Premium ──
    // Si la cuenta seleccionada es No-Premium, lanzar directamente en modo offline.
    if (m_accountToUse->accountType() == AccountType::Offline) {
        m_actualLaunchMode = LaunchMode::Offline;
        return LaunchDecision::Continue;
    }

    // Para cuentas Premium (MSA), verificar que tenga Minecraft comprado.
    ...
    if (state == AccountState::Working) {
        ...
        // Guardamos el shared_ptr ANTES de mostrar el diálogo para que no pueda
        // volverse null aunque la tarea termine mientras el diálogo está abierto.
    ...
    switch (state) {
        ...
        case AccountState::Expired:
            reauthReason = tr("'%1' ha expirado y necesita volver a autenticarse").arg(accountToCheck->profileName());
            break;
        case AccountState::Disabled:
            reauthReason = tr("La identificación del cliente del launcher ha cambiado");
            break;
        case AccountState::Gone:
            reauthReason = tr("'%1' ya no existe en los servidores").arg(accountToCheck->profileName());
            break;
        default:
            m_actualLaunchMode =
...
void LaunchController::launchInstance()
{
    ...
    } else if (m_actualLaunchMode == LaunchMode::Demo) {
        online_mode = "demo";
    } else {
        // Distinguir No-Premium (cuenta offline) de verdaderamente sin internet
        bool isNoPremium = m_accountToUse && m_accountToUse->accountType() == AccountType::Offline;
        online_mode = isNoPremium ? "no-premium" : "offline";
    }
