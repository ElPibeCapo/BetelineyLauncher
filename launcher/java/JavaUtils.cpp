...
#ifdef Q_OS_LINUX
    // Workaround para QTBUG-42500: QProcess en macOS no hereda PATH correctamente
    // desde Launch Services. Se fuerza el PATH del entorno de usuario manualmente.
...
{
    auto envVarName = QString("%1_JAVA_PATHS").arg(BuildConfig.LAUNCHER_APP_BINARY_NAME.toUpper());
    auto env = qEnvironmentVariable(envVarName.toUtf8().constData());
#if defined(Q_OS_WIN32)
...
QList<QString> JavaUtils::FindJavaPaths()
{
    ...
    candidates.append(getBetelineyJavaBundle());
...
QList<QString> JavaUtils::FindJavaPaths()
{
    ...
    javas.append(getBetelineyJavaBundle());
...
QList<QString> JavaUtils::FindJavaPaths()
{
    ...
    // javas stored in Beteliney Launcher's folder
    scanJavaDirs("java");
...
QStringList getBetelineyJavaBundle()
{
    QList<QString> javas;
