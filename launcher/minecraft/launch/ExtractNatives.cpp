...
{
    BetelineyZip::ArchiveReader zip(source);
    QDir directory(targetFolder);

    auto extPtr = BetelineyZip::ArchiveWriter::createDiskWriter();
    auto ext = extPtr.get();

    return zip.parse([applyJnilibHack, directory, ext](BetelineyZip::ArchiveReader::File* f) {
