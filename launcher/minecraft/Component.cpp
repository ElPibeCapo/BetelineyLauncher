...
{
    // If the metadata index hasn't loaded yet this returns nullptr.
    // Callers must handle the null case - the index loads asynchronously at startup.
    if (!APPLICATION->metadataIndex()) {
        qWarning() << "Component::getVersionList: metadata index not available yet for" << m_uid;
        return nullptr;
    }
...
QDateTime Component::getReleaseDateTime()
{
    ...
    // No release date available in either meta or version file.
    // Return current time as a safe fallback so sort-by-date doesn't crash.
...
{
    // All components are considered moveable for now.
    // A proper implementation would check dependency constraints to prevent
    // moving a component that another depends on being in a specific position.
    return true;
}
...
bool Component::customize()
{
    ...
    // Use try-catch because versionFileToJson may throw on malformed data.
    // Once the version format code is hardened to use error codes, this can be removed.
    try {
        QSaveFile jsonFile(filename);
        if (!jsonFile.open(QIODevice::WriteOnly)) {
            qWarning() << "Component::customize: could not open" << filename << "for writing";
            return false;
        }
        auto vfile = getVersionFile();
        if (!vfile) {
        ...
        if (!jsonFile.commit()) {
            qWarning() << "Component::customize: failed to commit" << filename;
            return false;
        }
        m_file = vfile;
        m_metaVersion.reset();
        ...
        qWarning() << "Component::customize: version file could not be serialized:" << error.cause();
        return false;
    }
    return true;
}
