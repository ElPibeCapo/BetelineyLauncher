...
static LoadResult loadComponent(ComponentPtr component, Task::Ptr& loadTask, Net::Mode netmode)
{
    ...
    if (QFile::exists(customPatchFilename)) {
        ...
        if (fileChanged) {
            if (!ProfileUtils::saveJsonFile(OneSixVersionFormat::versionFileToJson(file), customPatchFilename)) {
                qWarning() << "Failed to save modified component file:" << customPatchFilename;
            }
        }
