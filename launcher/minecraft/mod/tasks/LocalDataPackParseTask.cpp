...
bool processZIP(DataPack* pack, ProcessingLevel level)
{
    ...
    BetelineyZip::ArchiveReader zip(pack->fileinfo().filePath());
    ...
    if (!zip.parse(
            [&metaParsed, &iconParsed, &mcmeta_result, &pack_png_result, pack, level](BetelineyZip::ArchiveReader::File* f, bool& breakControl) {
                bool skip = true;
