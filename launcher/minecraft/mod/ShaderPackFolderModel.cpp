...
class ShaderPackIndexMigrateTask : public Task {
   ...
   public:
    ...
    void executeTask() override
    {
        ...
        if (!movedAll) {
            // Migration failure is logged here; the SequentialTask propagates it
            // via Task::failed signal. The UI will show a generic update error.
            qWarning() << "ShaderPack index migration: some files could not be moved to" << m_resourceDir.absolutePath();
