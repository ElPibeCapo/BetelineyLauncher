...
int Resource::compare(const Resource& other, SortType type) const
{
    switch (type) {
        ...
        case SortType::NAME: {
            ...
            // Strip leading "The " for natural sort (e.g. "The Aether" sorts under A)
            // If both names strip to the same string, compare returns 0 which is correct.
...
bool Resource::enable(EnableAction action)
{
    ...
    if (enable) {
        // m_enabled is false, but there's no '.disabled' suffix — unexpected state
        if (!path.endsWith(".disabled")) {
            qWarning() << "Resource::enable: expected .disabled suffix but not found:" << path;
            return false;
        }
        path.chop(9);
    } else {
