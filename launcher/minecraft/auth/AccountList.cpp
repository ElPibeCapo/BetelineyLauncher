...
void AccountList::onListChanged()
{
    if (m_autosave) {
        if (!saveList()) {
            emit saveFailure(tr("Failed to save the account list to disk. Your accounts may not persist after restarting."));
        }
    }
...
QString getAccountStatus(AccountState status)
{
    switch (status) {
        ...
        case AccountState::Gone:
            return QObject::tr("Removed", "Account status");
        default:
                ...
                case TypeColumn: {
                    switch (account->accountType()) {
                        case AccountType::MSA: {
                            return tr("Premium", "Account type");
                        }
                        case AccountType::Offline: {
                            return tr("No-Premium (online)", "Account type");
                        }
                    }
    ...
    switch (role) {
        ...
        case Qt::ToolTipRole:
            switch (section) {
                ...
                case TypeColumn:
                    return tr("Account type (Premium · MSA) or (No-Premium · online cracked servers)");
                case StatusColumn:
                    return tr("Current account status.");
                default:
                    return QVariant();
            }
...
bool AccountList::loadList()
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        qCritical() << QString("Failed to open account list file for reading (%1): %2")
...
bool AccountList::saveList()
{
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        qCritical() << QString("Failed to open account list file for writing (%1): %2")
                           .arg(m_listFilePath, file.errorString());
        return false;
    }
...
{
    // BETELINEY: Cuentas No-Premium son válidas sin necesitar una cuenta Premium.
    // Cualquier cuenta (Premium o No-Premium) hace que el launcher esté listo para jugar.
    for (auto account : m_accounts) {
