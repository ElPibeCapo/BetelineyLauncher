...
{
    BetelineyZip::ArchiveReader r(file.absoluteFilePath());

    m_isValid = false;
    r.parse([this](BetelineyZip::ArchiveReader::File* file, bool& stop) {
...
bool World::install(const QString& to, const QString& name)
{
    ...
    if (m_containerFile.isFile()) {
        BetelineyZip::ArchiveReader zip(m_containerFile.absoluteFilePath());
        ok = !BetelineyZip::extractSubDir(&zip, m_containerOffsetPath, finalPath);
