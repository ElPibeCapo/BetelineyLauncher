...
// Shared helper: load asset index and resolve standard paths
// Returns false if index file missing or unreadable
struct AssetPaths {
    QString indexPath;
    QDir objectDir;
    QDir virtualRoot;
...
bool loadAssetContext(const QString& assetsId, AssetPaths& out)
{
    QDir assetsDir("assets/");
    QDir indexDir(FS::PathCombine(assetsDir.path(), "indexes"));
    out.objectDir = QDir(FS::PathCombine(assetsDir.path(), "objects"));
    out.virtualRoot = QDir(FS::PathCombine(assetsDir.path(), "virtual", assetsId));
    out.indexPath = FS::PathCombine(indexDir.path(), assetsId + ".json");
    return QFile::exists(out.indexPath);
}
...
bool loadAssetsIndexJson(const QString& assetsId, const QString& path, AssetsIndex& index)
{
    ...
    */
    ...
    // El error se reporta a través de qCritical — la tarea que llama a esta función
    // debe verificar el valor de retorno y propagar el error a la UI.
...
// Refactored: shared loading logic extracted to loadAssetContext()
QDir getAssetsDir(const QString& assetsId, const QString& resourcesFolder)
{
    AssetPaths ctx;
    if (!loadAssetContext(assetsId, ctx)) {
        qCritical() << "No assets index file" << ctx.indexPath << "; can't determine assets path!";
        return ctx.virtualRoot;
    }

    AssetsIndex index;
    if (!AssetsUtils::loadAssetsIndexJson(assetsId, ctx.indexPath, index)) {
        qCritical() << "Failed to load asset index file" << ctx.indexPath << "; can't determine assets path!";
        return ctx.virtualRoot;
    }
...
bool reconstructAssets(QString assetsId, QString resourcesFolder)
{
    AssetPaths ctx;
    if (!loadAssetContext(assetsId, ctx)) {
        qCritical() << "No assets index file" << ctx.indexPath << "; can't reconstruct assets!";
        return false;
    }

    qDebug() << "reconstructAssets indexPath=" << ctx.indexPath
             << "virtualRoot=" << ctx.virtualRoot.path()
             << "objectDir=" << ctx.objectDir.path();

    AssetsIndex index;
    if (!AssetsUtils::loadAssetsIndexJson(assetsId, ctx.indexPath, index)) {
        qCritical() << "Failed to load asset index file" << ctx.indexPath << "; can't reconstruct assets!";
        return false;
    }
    ...
    if (index.isVirtual) {
        targetPath = ctx.virtualRoot.path();
    ...
    if (!targetPath.isNull()) {
        ...
        for (QString map : index.objects.keys()) {
            ...
            QString original_path = FS::PathCombine(ctx.objectDir.path(), tlk, asset_object.hash);
