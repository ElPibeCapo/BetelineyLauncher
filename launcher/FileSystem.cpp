...
bool overrideFolder(QString overwritten_path, QString override_path)
{
    ...
    std::error_code err;
    // std::filesystem::copy with overwrite_existing does NOT reliably overwrite on Windows
    // with GNU libstdc++. Workaround: remove destination directory first, then copy.
#if defined(Q_OS_WIN)
    fs::remove_all(StringUtils::toStdString(overwritten_path), err);
    if (err) {
        qCritical() << "Failed to remove override destination before copy:" << QString::fromStdString(err.message());
        return false;
    }
    err.clear();
#endif
