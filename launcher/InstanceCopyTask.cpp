...
void InstanceCopyTask::copyFinished()
{
    ...
    // Load instance settings and verify the file is readable
    auto instanceSettings = std::make_unique<INISettingsObject>(FS::PathCombine(m_stagingPath, "instance.cfg"));
    if (!QFileInfo(FS::PathCombine(m_stagingPath, "instance.cfg")).exists()) {
        emitFailed(tr("Instance configuration file not found after copy."));
        return;
    }
