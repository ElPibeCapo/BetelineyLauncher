...
Application::Application(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    /*
     * Establish the mechanism for communication with an already running BetelineyLauncher that uses the same data path.
    ...
    {
        // Note: the peer ID is based on path+version, so two instances from the same binary
        // but with different data dirs won't detect each other. This is a known limitation
        // that could cause update conflicts if both are running during an update.
    ...
    // Initialize application settings
    {
        ...
        // Theming
        m_settings->registerSetting("IconTheme", QStringLiteral("beteliney"));
        m_settings->registerSetting("ApplicationTheme", QStringLiteral("beteliney"));  // Beteliney
        ...
        // Ryzen 7 3700U tiene 8 threads lógicos — 6 tasks deja 2 libres para el SO y la UI.
        {
            int cores = QThread::idealThreadCount();
            int concurrentTasks = qMax(4, cores - 2);
            m_settings->registerSetting("NumberOfConcurrentTasks", concurrentTasks);
        }
        // Beteliney: descargas concurrentes adaptativas según RAM disponible.
        // En iGPU con 8-16 GB compartidos, más de 6 hilos de descarga satura el bus de memoria.
        // En sistemas con >16 GB, 10 hilos aprovechan mejor el ancho de banda.
        {
            int ramGB = static_cast<int>(HardwareInfo::totalRamMiB() / 1024);
            int concurrentDownloads = (ramGB >= 16) ? 10 : (ramGB >= 8) ? 6 : 4;
            m_settings->registerSetting("NumberOfConcurrentDownloads", concurrentDownloads);
        }
        m_settings->registerSetting("NumberOfManualRetries", 3);   // Beteliney: 3 reintentos (default 1 era insuficiente en conexiones inestables)
        m_settings->registerSetting("RequestTimeout", 120);        // Beteliney: 120s (default 60s era insuficiente para mods grandes)
        ...
        m_settings->registerSetting("AutoCloseConsole", true);  // Beteliney
        ...
        // Memory
        m_settings->registerSetting({ "MinMemAlloc", "MinMemoryAlloc" }, 512);   // Beteliney: 512 MB es suficiente con G1GC; el heap crece dinámicamente
        ...
        // Beteliney: flags por defecto = perfil "Balanceado" de BetelineyProfiles.h
        // G1HeapRegionSize=4M es correcto para heaps de 1–4 GB (4096/4 = 1024 regiones G1).
        // NO se incluyen ConcGCThreads ni ParallelGCThreads: la JVM los calcula automáticamente
        // según los núcleos del sistema — hardcodearlos perjudica en CPUs de 2 o de 32 núcleos.
        // El usuario puede cambiar los flags en Ajustes de Java o usando el selector de perfiles.
        m_settings->registerSetting("JvmArgs",
            "-XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200"
            " -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC"
            " -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=4M"
            " -XX:G1ReservePercent=20 -XX:G1MixedGCCountTarget=4"
            " -XX:InitiatingHeapOccupancyPercent=15 -XX:G1MixedGCLiveThresholdPercent=90"
            " -XX:G1RSetUpdatingPauseTimePercent=5 -XX:GCTimeRatio=99"
            " -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1"
            // UseStringDeduplication omitido en defaults: en 8 GB con max 2 GB para MC no justifica el thread de dedup
            " -XX:MinHeapFreeRatio=10 -XX:MaxHeapFreeRatio=20"
            " -Xss1M -Dusing.aikars.flags=https://mcflags.emc.gs -Daikars.new.flags=true");
        ...
        // Beteliney: en Linux con gamemode instalado (Nobara lo incluye), activar por defecto.
        // En Windows y macOS siempre false (gamemode no existe).
#ifdef Q_OS_LINUX
        m_settings->registerSetting("EnableFeralGamemode", true);
#else
        ...
        // Migración de datos: versiones antiguas guardaban la URL del pastebin como string plano.
        // Las versiones nuevas usan un enum (PasteType) + campo separado para URL custom.
        // Este bloque migra silenciosamente la configuración antigua al nuevo formato.
        {
        ...
        // Beteliney: limitar caché de pixmaps según RAM disponible.
        // Sin límite explícito Qt usa un default bajísimo (10 MB) o ilimitado según versión.
        // Con 16 GB y muchos íconos de mods, 128 MB evita redecodificaciones constantes.
        {
            int ramMiB = static_cast<int>(HardwareInfo::totalRamMiB());
            int cacheMiB = (ramMiB >= 12288) ? 128 : (ramMiB >= 8192) ? 64 : 32;
            QPixmapCache::setCacheLimit(cacheMiB * 1024);  // setCacheLimit toma KB
        }
        // Beteliney: el thread pool global de Qt también lo usan los tasks de descarga
        // y procesamiento de íconos. Ryzen 7 3700U tiene 8 threads lógicos.
        {
            int cores = QThread::idealThreadCount();
            QThreadPool::globalInstance()->setMaxThreadCount(qMax(4, cores));
        }
    ...
    // Instance icons
    {
        ...
        QStringList instFolders = { ":/icons/beteliney-icons/32x32/instances/", ":/icons/beteliney-icons/50x50/instances/",
                                    ":/icons/beteliney-icons/128x128/instances/", ":/icons/beteliney-icons/scalable/instances/" };
    ...
    // and accounts
    {
        ...
        connect(m_accounts.get(), &AccountList::saveFailure, this, [](const QString& reason) {
            qCritical() << "Account list save failure:" << reason;
            QMessageBox::warning(nullptr, tr("Account Save Error"), reason);
        });
    ...
    // Beteliney: traducciones hardcodeadas, no se descargan online
    // m_translations->downloadIndex();

    // Register available Java profilers - these are kept as factory objects
    // in the profilers map so the ExternalToolsPage can enumerate them
    ...
    // check update locks
    {
        auto update_log_path = FS::PathCombine(m_dataPath, "logs", "beteliney_update.log");

        auto update_lock = QFileInfo(FS::PathCombine(m_dataPath, ".beteliney_update.lock"));
        if (update_lock.exists()) {
            ...
            auto infoMsg = tr("This installation has a update lock file present at: %1\n"
                              ...
                              "proceeding.\n"
                              "Check the Beteliney updater log at: \n"
                              "%7\n"
        ...
        auto update_fail_marker = QFileInfo(FS::PathCombine(m_dataPath, ".beteliney_update.fail"));
        ...
        auto update_success_marker = QFileInfo(FS::PathCombine(m_dataPath, ".beteliney_update.success"));
...
bool Application::createSetupWizard()
{
    ...
    if (wizardRequired) {
        ...
        if (!validIcons)
            settings()->set("IconTheme", QString("beteliney"));
        if (!validWidgets) {
            settings()->set("ApplicationTheme", QStringLiteral("beteliney"));
        }
...
void Application::performMainStartupAction()
{
    ...
    if (!m_instanceIdToLaunch.isEmpty()) {
        ...
        if (inst) {
            ...
            if (!m_serverToJoin.isEmpty()) {
                QString trimmed = m_serverToJoin.trimmed();
                if (trimmed.isEmpty() || trimmed.contains(QRegularExpression("\\s"))) {
                    qWarning() << "Invalid server address (empty or contains whitespace):" << m_serverToJoin;
                } else {
                    targetToJoin.reset(new MinecraftTarget(MinecraftTarget::parse(trimmed, false)));
                    qDebug() << "   Launching with server" << trimmed;
                }
            } else if (!m_worldToJoin.isEmpty()) {
...
#endif
#else
        m_updater.reset(new BetelineyExternalUpdater(m_mainWindow, m_rootPath, m_dataPath));
#endif
