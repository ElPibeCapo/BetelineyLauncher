...
void ResourceUpdateDialog::checkCandidates()
{
    ...
    if (ui->modTreeWidget->topLevelItemCount() == 0) {
        ...
        // Sort top-level items alphabetically, children descending (newest first)
        // Disable sorting during manipulation to avoid repeated re-sorts
        ...
        for (int i = 0; i < ui->modTreeWidget->topLevelItemCount(); ++i) {
            ui->modTreeWidget->topLevelItem(i)->sortChildren(0, Qt::SortOrder::DescendingOrder);
        }
        ui->modTreeWidget->setSortingEnabled(true);
    }
