...
void ExportInstanceDialog::doExport()
{
    ...
    if (!BetelineyZip::collectFileListRecursively(m_instance->instanceRoot(), nullptr, &files,
    ...
    auto task = makeShared<BetelineyZip::ExportToZipTask>(output, m_instance->instanceRoot(), files, "", true);
