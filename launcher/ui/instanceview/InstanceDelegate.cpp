...
// Beteliney Gamer — dibuja toda la tarjeta (fondo + borde neón)
void drawGamerCard(QPainter* painter, const QStyleOptionViewItem& option, const QRect& fullRect)
{
    painter->save();
    ...
    QRect card = fullRect.adjusted(3, 3, -3, -3);

    // Fondo de la tarjeta
    QColor bg = selected ? QColor(0x0C, 0x1C, 0x0C) : QColor(0x14, 0x14, 0x14);
    painter->setBrush(bg);
    ...
    // Borde
    if (selected) {
        painter->setPen(QPen(QColor(0x39, 0xFF, 0x14), 2));
    } else if (hover) {
        painter->setPen(QPen(QColor(0x39, 0xFF, 0x14, 90), 1));
    } else {
...
// Beteliney Gamer — badge de texto en la parte inferior de la tarjeta
void drawSelectionRect(QPainter* painter, const QStyleOptionViewItem& option, const QRect& textRect)
{
    painter->save();
    painter->setRenderHint(QPainter::Antialiasing, true);
    ...
    QRect badge = textRect.adjusted(4, 2, -4, -2);

    if (selected) {
        painter->setBrush(QColor(0x39, 0xFF, 0x14));
        painter->setPen(Qt::NoPen);
        painter->drawRoundedRect(badge, 4, 4);
    } else {
        painter->setBrush(QColor(0x0A, 0x0A, 0x0A, 200));
        painter->setPen(Qt::NoPen);
        painter->drawRoundedRect(badge, 4, 4);
    }
...
void ListViewDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    // draw background
    {
        // Tarjeta gamer: fondo oscuro + borde neón
        drawGamerCard(painter, opt, opt.rect);
    ...
    if (cg == QPalette::Normal && !(opt.state & QStyle::State_Active))
        cg = QPalette::Inactive;
    if (opt.state & QStyle::State_Selected) {
        painter->setPen(QColor(0x00, 0x00, 0x00));  // negro sobre badge neón
    } else {
