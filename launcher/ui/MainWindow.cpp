...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    // hide, disable and show stuff
    {
        ...
        ui->actionOpenWiki->setVisible(!BuildConfig.WIKI_URL.isEmpty());
    ...
    // Beteliney: ocultar barra si no hay RSS configurado
    ...
    // load the news
    {
        if (m_newsChecker) m_newsChecker->reloadNews();
        updateNewsLabel();
    }
...
void MainWindow::processURLs(QList<QUrl> urls)
{
    ...
    for (auto& url : urls) {
        ...
        if (!url.isLocalFile()) {  // download the remote resource and identify
            // Beteliney: detectar import externo via URL protocol
            ...
            if (url.scheme() == "curseforge") {
                ...
                continue;
            } else if ((url.scheme() == "beteliney" || url.scheme() == BuildConfig.LAUNCHER_APP_BINARY_NAME)
                        && isExternalURLImport) {
                // Beteliney URL protocol modpack import
                // beteliney://import?url=ENCODED  o  beteliney://import/ENCODED
...
void MainWindow::globalSettingsClosed()
{
    ...
    // but Beteliney Launcher exits abnormally, causing the window state to never be saved:
