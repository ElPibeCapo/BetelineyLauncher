...
void VersionPage::on_actionDownload_All_triggered()
{
    if (!APPLICATION->accounts()->anyAccountIsValid()) {
        CustomMessageBox::selectable(this, tr("Error"),
                                     tr("Cannot download Minecraft or update instances unless you have at least "
                                        "one account added (Microsoft or Offline)."),
                                     QMessageBox::Warning)
            ->show();
        return;
    }
    ...
    auto result = tDialog.execWithTask(task.get());
    if (result != QDialog::Accepted)
        qDebug() << "Version update task did not complete successfully";
    updateButtons();
    m_container->refreshContainer();
}
...
void VersionPage::on_actionCustomize_triggered()
{
    ...
    if (!patch->getVersionFile()) {
        QMessageBox::warning(this, tr("Not Ready"),
                             tr("The component is still loading. Please wait for the update to finish before customizing."));
        return;
    }
    if (!m_profile->customize(version)) {
        QMessageBox::critical(this, tr("Customize Failed"),
                              tr("Failed to customize the selected component. Check the logs for details."));
    }
    updateButtons();
...
void VersionPage::on_actionRevert_triggered()
{
    ...
    if (!m_profile->revertToBase(version)) {
        QMessageBox::critical(this, tr("Revert Failed"),
                              tr("Failed to revert the selected component. Check the logs for details."));
    }
    updateButtons();
    preselect(currentIdx);
    m_container->refreshContainer();
