...
void ExternalResourcesPage::removeItem()
{
    ...
    if (multiple) {
        text = tr("Vas a eliminar %1 elementos.\n"
                  "Esto puede ser permanente y desaparecerán de la carpeta.\n\n"
                  "¿Seguro?")
                   .arg(count);
    } else if (folder) {
        text = tr("Vas a eliminar la carpeta \"%1\".\n"
                  "Esto puede ser permanente y desaparecerá de la carpeta superior.\n\n"
                  "¿Seguro?")
                   .arg(m_model->at(selection.indexes().at(0).row()).fileinfo().fileName());
    }

    if (!text.isEmpty()) {
        auto response = CustomMessageBox::selectable(this, tr("Confirmar eliminación"), text, QMessageBox::Warning,
...
void ExternalResourcesPage::removeItems(const QItemSelection& selection)
{
    if (m_instance != nullptr && m_instance->isRunning()) {
        auto response = CustomMessageBox::selectable(this, tr("Confirmar eliminación"),
                                                     tr("Si eliminás este recurso mientras el juego está corriendo puede crashear.\n"
...
QString ExternalResourcesPage::extraHeaderInfoString()
{
    if (ui && ui->treeView && ui->treeView->selectionModel()) {
        ...
        if (auto count = std::count_if(selection.cbegin(), selection.cend(), [](auto v) { return v.column() == 0; }); count != 0)
            return tr(" (%1 instalados, %2 seleccionados)").arg(m_model->size()).arg(count);
    }
    return tr(" (%1 instalados)").arg(m_model->size());
}
