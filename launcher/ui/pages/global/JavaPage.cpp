...
JavaPage::JavaPage(QWidget* parent) : QWidget(parent), ui(new Ui::JavaPage)
{
    ...
    if (BuildConfig.JAVA_DOWNLOADER_ENABLED) {
        ...
        ui->managedJavaList->setEmptyString(tr("No hay versiones de Java administradas instaladas"));
        ui->managedJavaList->setEmptyErrorString(tr("¡No se pudo cargar la lista de Java administrado!"));
    } else
    ...
    for (auto& entry : entries) {
        if (dcast->path.startsWith(entry.canonicalFilePath())) {
            auto response = CustomMessageBox::selectable(this, tr("Confirmar eliminación"),
                                                         tr("Estás a punto de eliminar la instalación de Java \"%1\".\n¿Estás seguro?")
                                                             .arg(entry.fileName()),
