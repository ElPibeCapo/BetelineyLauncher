...
AccountListPage::AccountListPage(QWidget* parent) : QMainWindow(parent), ui(new Ui::AccountListPage)
{
    ...
    ui->listView->setEmptyString(
        tr("¡Bienvenido!\n"
           "Podés agregar una cuenta Premium (Microsoft) o una cuenta No-Premium para jugar sin licencia."));
...
{
    ChooseOfflineNameDialog dialog(tr("Ingresá el nombre de usuario que querés usar para tu cuenta No-Premium."), this);
    ...
    if (const MinecraftAccountPtr account = MinecraftAccount::createOffline(dialog.getUsername())) {
        ...
        if (progDialog.execWithTask(loginTask.get()) == QDialog::Rejected) {
            CustomMessageBox::selectable(this, tr("Error"),
                tr("No se pudo crear la cuenta No-Premium."), QMessageBox::Critical)->exec();
            return;
        }
        m_accounts->addAccount(account);
        if (m_accounts->count() == 1)
...
{
    auto response = CustomMessageBox::selectable(this, tr("¿Eliminar cuenta?"), tr("¿Realmente querés eliminar esta cuenta?"),
