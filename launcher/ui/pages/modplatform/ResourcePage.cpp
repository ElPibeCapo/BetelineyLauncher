...
void ResourcePage::openedImpl()
{
    ...
    m_ui->searchEdit->setPlaceholderText(tr("Buscar %1...").arg(resourcesString()));
    m_ui->resourceSelectionButton->setText(tr("Seleccionar %1 para descargar").arg(resourceString()));
...
void ResourcePage::updateUi(const QModelIndex& index)
{
    ...
    if (current_pack->extraDataLoaded) {
        if (current_pack->extraData.status == "archived") {
            text += "<br><br>" + tr("Este proyecto fue archivado. No recibirá más actualizaciones a menos que el autor decida desarchivarlo.");
        }

        if (!current_pack->extraData.donate.isEmpty()) {
            text += "<br><br>" + tr("Información de donaciones: ");
        ...
        if (!current_pack->extraData.issuesUrl.isEmpty() || !current_pack->extraData.sourceUrl.isEmpty() ||
            ...
            text += "<br><br>" + tr("Links externos:") + "<br>";
        }
        ...
        if (!current_pack->extraData.sourceUrl.isEmpty())
            text += "- " + tr("Código fuente: <a href=%1>%1</a>").arg(current_pack->extraData.sourceUrl) + "<br>";
...
void ResourcePage::updateSelectionButton()
{
    ...
    if (auto current_pack = getCurrentPack(); current_pack) {
        if (current_pack->versionsLoaded && current_pack->versions.empty()) {
            ...
            m_ui->resourceSelectionButton->setText(tr("Deseleccionar %1 para descargar").arg(resourceString()));
    } else {
...
void ResourcePage::versionListUpdated(const QModelIndex& index)
{
    if (index == m_ui->packView->currentIndex()) {
        ...
        if (m_ui->versionSelectionBox->count() == 0) {
            m_ui->versionSelectionBox->addItem(tr("No hay versiones válidas."), QVariant(-1));
            m_ui->resourceSelectionButton->setText(tr("No se puede seleccionar una versión inválida :("));
        }
...
void ResourcePage::onSelectionChanged(QModelIndex curr, [[maybe_unused]] QModelIndex prev)
{
    ...
    if (!current_pack || !current_pack->versionsLoaded) {
        m_ui->resourceSelectionButton->setText(tr("Cargando versiones..."));
        m_ui->resourceSelectionButton->setEnabled(false);
        ...
        if (pack->isAnyVersionSelected())
            ...
            if (version == pack->versions.end()) {
                auto errorMessage = new QMessageBox(
                    QMessageBox::Warning, tr("Sin versiones disponibles"),
                    tr("No hay versiones disponibles para '%1'.\nEl autor probablemente bloqueó launchers de terceros.").arg(pack->name),
                    QMessageBox::Ok, this);
...
void ResourcePage::openProject(QVariant projectID)
{
    ...
    auto jump = [this] {
        ...
        m_ui->packDescription->setText(tr("El recurso no fue encontrado"));
    };
