...
QVariant ListModel::data(const QModelIndex& index, int role) const
{
    ...
    switch (role) {
        ...
        case Qt::ForegroundRole: {
            if (pack.broken) {
                // Use semantic red that works on light and dark themes
                return QApplication::palette().color(QPalette::Active, QPalette::HighlightedText).lighter() == Qt::white
                           ? QColor(200, 0, 40)   // light theme
                           : QColor(255, 80, 80);  // dark theme
            } else if (pack.bugged) {
                // bugged pack, currently only indicates bugged xml
                return QColor(Qt::yellow).darker(110);
            }
            return {};
        }
        case Qt::DisplayRole:
