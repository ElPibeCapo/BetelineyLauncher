...
struct Language {
    ...
    QString languageName() const
    {
        QString result;
        if (key == "en_US") {
            result = u8"Ingles (Estados Unidos)";
        } else if (key == "en_GB") {
            result = u8"Ingles (Británico)";
        } else if (key == "es_ES") {
            result = u8"Español (España)";
        } else if (key == "es_UY") {
            result = u8"Español (Latinoamérica)";
        } else {
...
void TranslationsModel::reloadLocalFiles()
{
    ...
    // Filtrar: solo se permiten estos 4 idiomas
    static const QSet<QString> allowedLanguages = { "en_US", "en_GB", "es_ES", "es_UY" };
    for (auto it = languages.begin(); it != languages.end();) {
        if (!allowedLanguages.contains(it.key())) {
            it = languages.erase(it);
        } else {
            ++it;
        }
    }
    // Asegurar que los 4 siempre estén presentes aunque no haya archivo de traducción
    for (const auto& key : allowedLanguages) {
        if (!languages.contains(key)) {
            languages.insert(key, Language(key));
        }
    }
