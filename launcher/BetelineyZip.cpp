...
bool createModdedJar(QString sourceJarPath, QString targetJarPath, const QList<Mod*>& mods)
{
    ...
    for (auto i = mods.crbegin(); i != mods.crend(); i++) {
        ...
        if (mod->type() == ResourceType::ZIPFILE) {
            ...
            // Skip if already added by a previous mod (prevents duplicate entries in jar)
            ...
            // Ruta FOLDER: usada cuando un mod es una carpeta en vez de un .jar.
            // La lógica de deduplicación (addedFiles) garantiza que no se añadan archivos
            // repetidos aunque dos mods de tipo FOLDER compartan archivos.
            auto filename = mod->fileinfo();
            ...
            // Filter out files already added (relative to parent_dir)
            QFileInfoList toAdd;
            for (const auto& e : files) {
                QString relPath = QDir(parent_dir).relativeFilePath(e.filePath());
                if (!addedFiles.contains(relPath))
                    toAdd.append(e);
            }

            if (!compressDirFiles(zipOut, parent_dir, toAdd)) {
                zipOut.close();
                FS::deletePath(targetJarPath);
                qCritical() << "Failed to add" << mod->fileinfo().fileName() << "to the jar.";
                return false;
            ...
            for (const auto& e : toAdd)
                addedFiles.insert(QDir(parent_dir).relativeFilePath(e.filePath()));
