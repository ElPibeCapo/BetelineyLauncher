...
{
    if (m_settings->get("lastLaunchTime").value<qint64>() == val)
        return;
...
{
    if (m_settings->get("notes").toString() == val)
        return;
...
{
    if (m_settings->get("iconKey").toString() == val)
        return;
...
{
    if (m_settings->get("name").toString() == val)
        return;
...
void BaseInstance::setShortcuts(const QList<ShortcutData>& shortcuts)
{
    ...
    QString newVal = QString::fromUtf8(document.toJson(QJsonDocument::Compact));
    if (m_settings->get("shortcuts").toString() == newVal)
        return;
    m_settings->set("shortcuts", newVal);
}
