...
auto NetRequest::handleRedirect() -> bool
{
    ...
    if (!redirect.isValid()) {
        ...
        /*
         * Qt doesn't parse the Location header in tolerant mode, which causes issues
         * with some servers that return non-RFC-compliant redirect URLs.
         * Workaround: parse manually with QUrl::TolerantMode.
         * Upstream Qt issue: https://bugreports.qt.io/browse/QTBUG-67947
         */
...
void NetRequest::downloadFinished()
{
    ...
    if (m_state != State::Succeeded) {
        ...
        m_sink->abort();
        if (m_checksumRetryCount < MAX_CHECKSUM_RETRIES) {
            m_checksumRetryCount++;
            qCWarning(logCat) << getUid().toString() << "Checksum mismatch, reintentando (" << m_checksumRetryCount << "/" << MAX_CHECKSUM_RETRIES << "):" << m_url.toString();
