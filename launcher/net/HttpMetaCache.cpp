...
auto HttpMetaCache::getEntry(QString base, QString resource_path) -> MetaEntryPtr
{
    ...
    if (!m_entries.contains(base)) {
        qWarning() << "HttpMetaCache: requested unknown base" << base << "for resource" << resource_path;
        return {};
    }
...
{
    if (m_entries.contains(base)) {
        qWarning() << "HttpMetaCache: tried to add duplicate base" << base << "- ignoring";
        return;
    }

    // BUG 5 fix: si la ruta no existe, intentar crearla. Si no se puede, no registrar la base.
    if (base_root.isEmpty()) {
        qCritical() << "HttpMetaCache: base path is empty for base" << base << "- not registering";
        return;
    }
    if (!QDir(base_root).exists()) {
        qWarning() << "HttpMetaCache: base path does not exist, attempting to create:" << base_root;
        if (!QDir().mkpath(base_root)) {
            qCritical() << "HttpMetaCache: failed to create base path" << base_root << "- not registering";
            return;
        }
    }
