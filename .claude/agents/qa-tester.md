...
Antes de arrancar, revisá tu memoria (`.claude/agent-memory/qa-tester/MEMORY.md` si existe) por patrones de fallas ya vistos (tests con `QSKIP` sospechoso, áreas con contaminación de estado entre casos, flujos manuales que ya se marcaron como no automatizables) — evita reconstruir esa cobertura desde cero cada vez. Al cerrar, anotá ahí cualquier patrón de falla nuevo que valga la pena que la próxima corrida chequee de entrada.
